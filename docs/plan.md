...
The browser application is built with React, Vite, and TypeScript. Authentication is implemented with MSAL using the OAuth 2.0 Authorization Code Flow with PKCE. The resulting Entra access token is sent to an Amazon API Gateway HTTP API. API Gateway validates the token with a native JWT authorizer before a request can reach AWS Lambda. Lambda therefore receives an already authenticated request and is responsible for business authorization, input validation, domain logic, and integration with downstream AWS or enterprise services.
...
3. **Use access tokens for APIs.** ID tokens are for the client application to understand the authenticated session; access tokens are presented to API Gateway.
4. **Authenticate before compute.** API Gateway rejects invalid tokens before Lambda execution wherever possible.
...
6. **Infrastructure is reproducible.** AWS resources are created through CDK and validated through automated tests and CI/CD.
7. **Security controls are observable.** Authentication failures, API failures, throttling, Lambda errors, and deployment health must be measurable without logging sensitive tokens.
...
- Custom JWT signature validation inside Lambda unless there is a specific requirement that API Gateway cannot satisfy
...
The target architecture separates four concerns: frontend delivery, identity, API enforcement, and application execution. CloudFront and S3 deliver static application assets. Microsoft Entra ID performs user authentication and organizational access policy. API Gateway exposes the HTTPS API and validates Entra access tokens. Lambda executes application logic only after the request has passed the API authorization boundary.

This separation matters because it prevents the Lambda function from becoming an identity server. The function does not need to know how passwords were entered, how MFA was satisfied, or how signing keys are discovered. Instead, it consumes trusted authorization context after API Gateway has validated the token. This reduces custom security code and creates clearer ownership when diagnosing failures.

Operationally, the request path has two distinct planes. The **frontend plane** serves versioned static files through CloudFront. The **API plane** handles authenticated dynamic requests through API Gateway and Lambda. These planes can be deployed, scaled, monitored, and rolled back independently.
...
flowchart LR
    ...
    subgraph AWSAPI["AWS API Layer"]
        ...
        JWT["JWT Authorizer<br/>Entra Issuer + Audience"]
    ...
    User -->|"Bearer Access Token"| APIGW
    APIGW --> JWT
    JWT -->|"Authorized"| Lambda
...
**Entra issues the token → API Gateway validates the token → Lambda trusts only validated claims supplied by API Gateway.**

API Gateway validates the signing key, issuer, audience, token timestamps and optionally the `scope`/`scp` claim before the request reaches Lambda.
...
The scope is more than a label. API Gateway should require it on protected routes so that a structurally valid token without the intended API permission is still rejected. This also helps distinguish access tokens intended for this API from unrelated tokens issued by the same tenant.
...
MSAL should own protocol details such as constructing authorization requests, managing PKCE, handling redirects, maintaining account state, and obtaining tokens. Application code should call MSAL APIs rather than manually assembling OAuth requests.

After sign-in, the SPA acquires an access token for the application API scope and sends it as a bearer token to API Gateway. The browser should not send the ID token to the API as an authorization credential. API Gateway validates the access token and forwards validated claims to Lambda only when the configured checks succeed.
...
sequenceDiagram
    ...
    SPA->>API: GET /api/me<br/>Authorization: Bearer TOKEN
    API->>Auth: Validate JWT
    Auth->>Entra: Retrieve / cache JWKS
    Entra-->>Auth: Signing keys
    Auth->>Auth: Validate signature, iss, aud, exp, scp

    alt Valid
        Auth->>Lambda: Invoke + JWT claims
        Lambda-->>API: 200 response
        API-->>SPA: JSON
    else Invalid
        Auth-->>SPA: 401 / 403
    end
...
Use tenant-specific authority rather than broad multi-tenant endpoints for this single-tenant workload. This reduces ambiguity and aligns API Gateway issuer validation with the same tenant boundary.
...
It is not an API security boundary. Any attacker can bypass or modify browser-side JavaScript, so every sensitive backend action must still be protected by API Gateway authorization and Lambda business authorization.
...
Before each protected request, the client obtains an access token for the intended API scope through the token service and attaches it in the standard `Authorization: Bearer` header. The token must never be written to application logs, analytics events, error trackers, or browser-visible debug output in production.

HTTP status codes should be translated consistently. A 401 generally means the authentication credential is absent or no longer usable. A 403 means the caller is authenticated but not permitted. A 429 indicates throttling and may justify controlled backoff. 5xx responses represent server-side failures and should surface a correlation ID that support teams can trace without exposing internal details.
...
Amazon API Gateway HTTP API is the public API entry point. It terminates HTTPS, applies CORS and route configuration, performs JWT authorization, provides access logging and metrics, and invokes the Lambda integration only after configured authorization checks pass.

Using the native JWT authorizer minimizes custom token-processing code. API Gateway retrieves signing keys from the issuer metadata, validates the JWT signature and supported claims, verifies configured issuer and audience values, checks time-based claims, and can enforce route scopes.

Protected routes should require the API's delegated scope. This is important because a token being signed by Entra does not by itself prove that the token was intended to call this API.

Keep public endpoints deliberate. A health endpoint may be unauthenticated if required for external availability checks, but it must not expose configuration, dependency details, version secrets, environment variables, or sensitive system state.
...
rather than implementing authentication inside Lambda.

Configure a native JWT authorizer:
...
AWS API Gateway can use the issuer's JWKS keys to validate RSA-signed JWTs and verify `iss`, `aud`, `exp`, `nbf`, `iat`, and `scope`/`scp`.

CDK directly supports HTTP API JWT authorizers with issuer and audience configuration.
...
The API request path is a sequence of trust decisions. HTTPS protects the request in transit. The bearer token identifies the delegated caller and intended API resource. API Gateway verifies the token before invoking compute. Lambda then performs business authorization using validated claims and request data.

This ordering is intentional because the cheapest and most standardized rejection should happen first. Invalid, expired, wrongly issued, wrongly targeted, or incorrectly scoped tokens should be stopped at API Gateway. Lambda should spend execution time only on requests that have already passed authentication and coarse-grained API authorization.
...
Lambda is the application execution layer. Handlers should be thin adapters that parse API Gateway events, establish request context, validate inputs, call domain/service code, and translate results into HTTP responses. Business rules should live in testable service modules rather than large monolithic handlers.

The `auth` package should contain helpers for reading already validated JWT claims and converting them into application identity context. It should not duplicate API Gateway signature verification or implement a second OAuth stack.
...
API Gateway owns those responsibilities.
...
Retrieve validated claims from:

```text
event.requestContext.authorizer.jwt.claims
```

AWS documents that validated JWT claims are passed to the Lambda integration through the request authorizer context.
...
CDK assertions are part of architecture testing. They should verify security-critical properties such as private S3 access, JWT authorizer configuration, route authorization, IAM restrictions, log retention, and required encryption settings.
...
The API stack defines the external contract between the browser and backend. It contains API Gateway routes, JWT authorization, Lambda integrations, CORS policy, domain configuration, access logging, throttling-related controls, and the IAM permissions required for invocation and execution.

CORS is a browser security mechanism, not an authentication mechanism. Restrict allowed production origins to the actual frontend domains, allow only required methods and headers, and test preflight requests as part of integration testing.

Custom API domains should use managed TLS certificates and stable DNS names so frontend configuration does not depend on generated API Gateway identifiers. Environment-specific domains also make it easier to enforce separation between dev, test, and production.

Access logs should include request identifiers, route, status, integration latency, and authorization outcome where supported, while avoiding raw authorization headers or token contents.
...
Structured JSON logging allows CloudWatch Logs Insights or downstream tooling to filter and aggregate fields reliably. A correlation ID should enter at the API boundary and be propagated through Lambda and any downstream calls so a user-visible failure can be traced without reproducing the request.
...
Authentication testing should focus on how the application integrates with MSAL and handles identity states rather than attempting to test Microsoft's identity platform itself. JWT cryptography is similarly owned by Entra and API Gateway; application tests should verify authorizer configuration and how Lambda reacts to the claims it receives.
...
Lambda tests should construct representative API Gateway events containing validated authorizer claims and verify both successful and denied business operations. Domain services should be tested independently from AWS event plumbing so authorization and business rules remain easy to reason about.
...
Do not unit test Microsoft's JWT cryptography yourself.
...
Correct issuer
Correct audience
Protected routes have authorization
CORS restricted
...
A deployment is not complete when CDK returns success. Smoke tests and CloudWatch health signals should confirm that the frontend loads, protected API routes behave correctly, and no new error pattern has appeared.
...
For this serverless workload, the pillars interact. For example, moving JWT validation to API Gateway improves security by reducing custom cryptographic code, operational excellence by centralizing policy, performance by rejecting bad requests before compute, and cost by avoiding unnecessary Lambda invocations.
...
| Security               | Entra MFA and Conditional Access, PKCE, no SPA secrets, API Gateway JWT validation, scoped authorization, least-privilege IAM, private S3 |
| Reliability            | Managed Entra authentication, API Gateway, Lambda, CloudFront, automated deployment, alarms and health checks                             |
| Performance Efficiency | CloudFront caching, Lambda execution reuse, HTTP API, appropriately sized Lambda memory                                                   |
| Cost Optimization      | Serverless compute, HTTP API, S3 static hosting, CloudFront caching, log retention policies                                               |
...
Implementation is phased to reduce risk by proving the hardest trust boundary early. Identity and token shape are validated before significant UI or backend development. The repository and quality controls are established before code volume grows. API authorization is implemented before feature APIs are added. Observability and deployment automation are completed before the solution is considered production-ready.
...
Prove the Entra model first. Configure the single registration, expose the API scope, register local and production-style SPA redirects, obtain a v2 access token through MSAL, and inspect the token claims expected by API Gateway. This phase should resolve uncertainty about issuer, audience, scopes, token version, and tenant before AWS authorization is built around them.

**Exit criteria:** a developer can authenticate through Entra and obtain the intended access token without a client secret, and the documented claims match the values that will be configured in API Gateway.
...
Introduce the AWS trust boundary. Create the HTTP API, JWT authorizer, protected route, and Lambda integration. Validate the real Entra issuer, audience, and `access_as_user` scope against tokens produced in Phase 1.
...
**Exit criteria:** valid scoped tokens reach Lambda; missing, expired, wrong-audience, wrong-issuer, or incorrectly scoped tokens are rejected before Lambda.

Create:

```text
API Gateway HTTP API
Entra JWT Authorizer
Issuer validation
Audience validation
...
The architecture has three major authorization boundaries and they must not be collapsed into one another. Entra authenticates the human and enforces enterprise identity policy. API Gateway validates whether the presented access token can enter the API. Lambda decides whether the validated identity can perform the requested business operation.

This layered model limits the amount of trust placed in any single control. A frontend route cannot grant backend access. A valid Entra token cannot call an API for which it was not intended. A valid API token does not automatically grant access to every record or operation.
...
API Gateway is responsible for coarse-grained API admission. It validates the presented token against the configured issuer, audience, token lifetime, signature keys, and required route scopes. Requests that fail this admission policy should not consume Lambda application logic.
...
8. React sends the access token to API Gateway.
9. API Gateway validates Entra's JWT before Lambda executes.
10. Incorrect issuer, audience, expiry, or scope is rejected.
11. Lambda receives only validated claims.
...
16. CORS permits only approved frontend origins.
...
The final target is intentionally simple in its trust flow: React uses MSAL to obtain an Entra access token, API Gateway validates that token at the AWS API boundary, and Lambda performs application-specific authorization and business logic. CloudFront/S3 provide the separate static delivery path, while CDK, CI/CD, tests, and CloudWatch make the system reproducible and operable.
...
**API Gateway performs cryptographic token validation.**

**Lambda performs business authorization.**
...
**Guardrails:** Keep the application single-tenant; use Authorization Code Flow with PKCE; never add a client secret to the SPA; require explicit API scopes; validate issuer/audience/scopes at API Gateway; use stable claims for business authorization; maintain named owners; review permissions and redirect URIs regularly.
